...
class ScheduledScrapingService:
    ...
    async def import_sighting(self, sighting_data: SightingData, source: str) -> Optional[Sighting]:
        ...
        try:
            ...
            if "duplicate" in error_msg or "unique constraint" in error_msg:
                logger.debug(f"Skipped duplicate: {sighting_data.species_name} at {sighting_data.location_name}")
                return None
            else:
    ...
    async def scrape_region(
        ...
        logger.info(f"Scraping {region_name} ({region_code}) - last {days_back} days, max {max_results}")
        ...
        try:
            ...
            logger.info(f"Imported {imported_count}/{len(sightings_data)} sightings from {region_name}")
